...
### 衍生改动跟随它所属的能力 | A follow-on stays with the capability it came from

**一次改动如果是先前某个次版本号引入的能力在使用中衍生出来的——暴露出的问题、不得不补齐的环节、随之要调整的显示——一律计为修订号。**那次使用方式的变化在引入时已经计过一次，不重复计。

判断顺序：

1. 这次改动**能不能脱离前一个能力独立成立**？不能，就是衍生改动 → **修订号**。
2. 能独立成立的，再看它是**修掉一个错误行为**（→ 修订号）还是**多出一种新做法**（→ 次版本号）。

例子：v0.12.0 新增「再出 5 题」是次版本号；由此衍生的两次改动都是修订号——

- 刷新后已答的题被重判（v0.12.1）：增量判分是为加题而改的，这是它带出的问题；
- 空题不再按错判、因而可以分次提交（v0.13.0）：没有「一天可能做不完 30 题」就不会有这个需求，它补的是加题这件事本身缺的一环，不是另一种新用法。

> 已发布的版本号不重编。规则从写入之日起对新版本生效；此前 v0.9.0 与 v0.10.0（两批词汇导入）、以及 v0.13.0（按本条应为修订号）都已经发布，保持原样。
